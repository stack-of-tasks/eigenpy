...
template <typename T>
void set_all_entries_to_constant(
    Eigen::Ref<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> M,
    T const& the_constant) {
  for (Eigen::Index ii = 0; ii < M.rows(); ++ii) {
    for (Eigen::Index jj = 0; jj < M.cols(); ++jj) {
...
a_function_taking_both_a_scalar_and_a_vector(
    T const& scalar,
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> const& M) {
  Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> result;
  set_all_entries_to_constant(
      Eigen::Ref<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>>{result},
      scalar);
  return result;
}
...
void ExposeAll() {
  ...
  // some free C++ functions that do stuff to Eigen::Matrix types.  useful to
  // prove they work.
  boost::python::def("set_to_ones", &set_to_ones<mpfr_float>,
                     "set an array to all ones");
  boost::python::def("set_to_ones", &set_to_ones<mpfr_complex>,
                     "set an array to all ones");

  boost::python::def(
      "set_all_entries_to_constant", &set_all_entries_to_constant<mpfr_float>,
      "set an array to all one value, from a given number of the same type");
  boost::python::def(
      "set_all_entries_to_constant", &set_all_entries_to_constant<mpfr_complex>,
      "set an array to all one value, from a given number of the same type");

  boost::python::def("make_a_vector_in_cpp", &make_a_vector_in_cpp<mpfr_float>,
                     "make a vector from c++");
  boost::python::def("make_a_vector_in_cpp",
                     &make_a_vector_in_cpp<mpfr_complex>,
                     "make a vector from c++");

  boost::python::def("a_function_taking_both_a_scalar_and_a_vector",
                     &a_function_taking_both_a_scalar_and_a_vector<mpfr_float>,
                     "do nothing, but accept both a scalar and a vector");
  boost::python::def(
      "a_function_taking_both_a_scalar_and_a_vector",
      &a_function_taking_both_a_scalar_and_a_vector<mpfr_complex>,
      "do nothing, but accept both a scalar and a vector");
